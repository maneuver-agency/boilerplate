...
let mnvrLoadMore = window.mnvr_loadmore || {}
...
LoadMore.prototype = {
  ...
  hasItemsLeft: function () {
    ...
    return mnvrLoadMore.count[this.settings.posttype] &&
      mnvrLoadMore.posts_per_page * page < mnvrLoadMore.count[this.settings.posttype].publish
  },
  ...
  fetch: function (init) {
    ...
    var data = {
      nonce: mnvrLoadMore.nonce,
      action: 'mnvr_ajax_load_more',
      page: page
    }

    if (init) {
      data.posts_per_page = page * mnvrLoadMore.posts_per_page
    }
    ...
    $.post(mnvrLoadMore.url, data, function (res) {
